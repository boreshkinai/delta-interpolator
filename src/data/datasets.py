...
def download_and_unzip(local_dir, dataset_name, url):
	...
	if len(url) > 0 and (not os.path.isdir(target_dir) or not os.path.isfile(check_file)):
		try:
			...
			pass
		logging.info("downloading dataset: %s from %s into %s..." % (local_dir, url, target_dir))
		...
		logging.info("extracting dataset to %s..." % target_dir)
...
class DatasetLoader():
	def __init__(self, dataset_path):
		self.known_datasets = {
			"deeppose_lafan_v1_fps30": "https://storage.googleapis.com/ml_artistry/deeppose_lafan_v1_fps30.zip",
		}
	...
	def pull_test_dataset(self) -> str:
		return self.pull("deeppose_test_dataset_v1")
...
class SplitFileDatabaseLoader(DatasetLoader):
	...
	def pull_test_dataset(self):
		return self.pull("deeppose_test_dataset_splitfile_v1")
