...
from src.modules.interpolator import InbetweenInterpolator
...
class Transformer(torch.nn.Module):
    ...
    def forward(self, input_data: Dict[str, torch.Tensor], *args) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        src_time = torch.unique((torch.cat([input_data['past_frame_indices'], 
                         input_data['key_frame_indices'], 
                         input_data['future_frame_indices']])), sorted=True).to(dtype=torch.int64)

        tgt_time = input_data["target_frame_indices"].to(dtype=torch.int64)

        joint_positions_interpolator = self.interpolator.forward(input_data)
        
        joint_positions_ref = input_data["joint_positions"][:, 0, :].unsqueeze(1)

        start_time = 0
        end_time = None 
        if input_data["past_frame_indices"] in input_data["key_frame_indices"]:
            start_time = 1
        if input_data["future_frame_indices"] in input_data["key_frame_indices"]:
            end_time = -1
        if self.input_delta_mode == 'none':
            x_src = input_data["joint_positions"][:, start_time:end_time]
            x_src = x_src.view(*x_src.shape[:2], -1)
            x_tgt = joint_positions_interpolator
            x_tgt = x_tgt.view(*x_tgt.shape[:2], -1)
        elif self.input_delta_mode == 'last_pose':
            x_src = input_data["joint_positions"][:, start_time:end_time] - joint_positions_ref
            x_src = x_src.view(*x_src.shape[:2], -1)
            x_tgt = 0.0 * (joint_positions_interpolator[:, input_data["target_frame_indices"]] - joint_positions_ref)
            x_tgt = x_tgt.view(*x_tgt.shape[:2], -1)
        ...
        stage2 = stage2 + joint_positions_interpolator
