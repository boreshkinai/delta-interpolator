...
from src.data.frame_sampler import PeriodicFramesRemover
from src.data.batched_sequence_dataset import AnidanceSequenceDataset


VALID_BENCHMARKS = {"anidance": "anidance"}


class InFillBenchmarkEvaluator:
    def __init__(self, 
        ...
        for n_trans in [5, 15, 30]:
            self.frame_samplers[n_trans] = PeriodicFramesRemover(past_context=self.n_past, 
    ...
    def evaluate(self, model):
        ...
        for n_trans in [5, 15, 30]:
            ...
            for i in range(len(self.validation_dataset)):
                ...
                past_frames, future_frames, target_frames, key_frames = model.get_data_from_batch(b, frame_sampler=self.frame_samplers[n_trans])

                target_data, predicted = model.forward_wrapped(past_frames, future_frames, target_frames, key_frames)
