...
from src.data.frame_sampler import PeriodicFramesRemoverOptions, RandomPeriodicFramesRemoverOptions
...
from src.evaluation.infilling_benchmark import InFillBenchmarkEvaluator


@dataclass
class AnidanceInFillModelOptions(BaseOptions):
    frame_sampler: Any = RandomPeriodicFramesRemoverOptions(past_context=1,
                                                            future_context=1,
                                                            min_middle_frames=4,
...
@ModelFactory.register(AnidanceInFillModelOptions, schema_name="AnidanceInFill")
class AnidanceInFillModel(LightningModule):

    def __init__(self, opts: AnidanceInFillModelOptions):
        super().__init__()
        ...
        if opts.benchmark != 'None':
            self.evaluator = InFillBenchmarkEvaluator(datasets_path=hydra.utils.to_absolute_path(opts.datasets_path),
    ...
    def forward_wrapped(self, past_frames, future_frames, target_frames, key_frames):
        """
    ...
    def prepare_inputs(self, input_data):
        ...
        if type(input_data['past_joint_positions']) is dict:
            joint_positions = torch.cat([input_data['past_joint_positions']['joint_positions'], input_data['key_frame_joint_positions']['joint_positions'], input_data['future_joint_positions']['joint_positions']], dim=1)
        else:
            joint_positions = torch.cat([input_data['past_joint_positions'], input_data['key_frame_joint_positions'], input_data['future_joint_positions']], dim=1)
        input_frame_indices = torch.cat([input_data['past_frame_indices'], input_data['key_frame_indices'], input_data['future_frame_indices']], dim=0)
        ...
        new_input_dict = {
            ...
            'key_frame_indices': input_data['key_frame_indices']
        }
    ...
    def shared_step(self, batch):
        past_frames, future_frames, target_frames, key_frames = self.get_data_from_batch(batch, self.frame_sampler)
        
        target_data, predicted = self.forward_wrapped(past_frames, future_frames, target_frames, key_frames)
        ...
        key_frames = self.expand_input(key_frames)
        ...
        src_time = past_frames['frame_indices'].tolist() + key_frames['frame_indices'].tolist() + future_frames['frame_indices'].tolist()
        src_time = sorted(set(src_time))

        src_time = torch.cat([past_frames['frame_indices'], key_frames['frame_indices'], future_frames['frame_indices']], dim=0)
        tgt_time = target_frames["frame_indices"]

        context_positions_global = torch.cat([past_frames['joint_positions'], 
                                              key_frames['joint_positions'],
    ...
    def test_step(self, batch, batch_idx):
        ...
        past_frames, future_frames, target_frames, key_frames = self.get_data_from_batch(batch, frame_sampler=self.frame_sampler)
        target_data, predicted_data = self.forward_wrapped(past_frames, future_frames, target_frames, key_frames)
    ...
    def get_batch_frame_indices(self, batch_length, training=True):
        ...
        past_indices, future_indices, output_indices, key_frame_indices = self.frame_sampler(list(range(batch_length)))
        return past_indices, future_indices, output_indices, key_frame_indices

    def get_data_from_batch(self, batch: Dict[str, torch.Tensor], frame_sampler: Any):
        past_indices, future_indices, output_indices, key_frame_indices = frame_sampler(list(range(batch['sequence_length'])))
        ...
        key_frame_indices = torch.Tensor(key_frame_indices).to(dtype=torch.int64, device=self.device)
        ...
        key_frames = self.get_subsequence_from_batch(batch, key_frame_indices)
        ...
        key_frames['frame_indices'] = key_frame_indices - start_frame

        # print(past_frames)

        return past_frames, future_frames, target_frames, key_frames

    def get_dummy_input(self):
        return {
            "past_frame_indices": torch.zeros((1), dtype=torch.int64),
            "future_frame_indices": torch.zeros((1), dtype=torch.int64),
            "output_frame_indices": torch.zeros((30), dtype=torch.int64),
            "key_frame_indices": torch.zeros((5), dtype=torch.int64),
            "past_joint_positions": torch.randn((1, 1, 3 * 24)),
            "future_joint_positions": torch.randn((1, 1, 3 * 24)),
            "key_frame_joint_positions": torch.randn((1, 5, 3 * 24))
        }
    ...
    def get_dynamic_axes(self):
        return {
            ...
            'key_frame_indices': {0: 'number_of_key_frames'},
            ...
            'key_frame_joint_positions': {1: 'number_of_key_frames'}
        }
