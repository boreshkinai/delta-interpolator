...
from src.geometry.forward_kinematics import forward_kinematics, \
    extract_translation_rotation, forward_kinematics_quats
from src.geometry.inverse_kinematics import inverse_kinematics, inverse_kinematics_quats
...
class Skeleton:
    ...
    def _add_missing_joints(self, skeleton_data):
        ...
        for joint in fixed_skeleton_data["joints"]:
            ...
            if parent_bone is not None and parent_bone not in all_proximal_joints:
    ...
    def forward_quats(self, joints_local_rotations, true_hip_offset=None):
        ...
        offsets = self.joint_offsets.type_as(joints_local_rotations).view(1, joints_nbr, 3).repeat(batch_size, 1, 1)  # (batch, joints, 3)
    ...
    # out: world positions of each joint in the batch (B, J, 3)
    # note: for now only works with only one parent bone at index 0 (hips)
    def invert(self, joints_global_rotations: torch.Tensor, joints_global_positions: torch.Tensor, true_hip_offset=None):
        batch_size = joints_global_rotations.shape[0]
        joints_nbr = self.joint_offsets.shape[0]

        if true_hip_offset is not None:
            joints_global_positions = joints_global_positions - true_hip_offset[...,None,:].expand_as(joints_global_positions)
    ...
    # note: for now only works with only one parent bone at index 0 (hips)
    def invert_quats(self, joints_global_rotations: torch.Tensor, joints_global_positions: torch.Tensor, true_hip_offset=None):
        if true_hip_offset is not None:
            joints_global_positions = joints_global_positions - true_hip_offset[..., None, :].expand_as(joints_global_positions)
