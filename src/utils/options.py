...
from src.utils.versioning import get_git_commit_id
...
class CheckpointOptions:
    last: Optional[bool] = None  # True if last model should be always checkpointed
    top_k: int = 1  # number of best checkpoints to keep (if 1 and monitor is None, will save last model)
    monitor: Optional[str] = None  # the metric to monitor for saving best checkpoints
    mode: str = "min"  # "min" or "max" to indicate when a metric is better than another
    every_n_epochs: Optional[int] = None  # checkpoints period
